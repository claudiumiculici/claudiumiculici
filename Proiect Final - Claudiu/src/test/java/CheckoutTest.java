...
public class CheckoutTest extends Hooks {
    ...
    @Test (description = "This test will order a product from the checkout page as a guest user.")
    public void CheckoutTestAsAGuest() throws InterruptedException {
        checkoutPage.CheckoutTestAsAUser();
        assertEquals("Continue shopping",checkoutPage.GetContinueShoppingButton().getText());
        Thread.sleep(3000);
        ExtentTestNGITestListener.getTest().log(Status.INFO,"A product has been added to the cart and the checkout page has been tested.");
    }

    @Test (description = "This test will order multiple products as a guest user.")
    public void MultipleProductsCheckoutGuestTest() throws InterruptedException {
        checkoutPage.MultipleProductsCheckoutGuestTest();
        assertEquals("Continue shopping", checkoutPage.GetContinueShoppingButton().getText());
        Thread.sleep(3000);
        ExtentTestNGITestListener.getTest().log(Status.INFO,"Multiple products have been added to the cart and the checkout page was tested.");
    }
